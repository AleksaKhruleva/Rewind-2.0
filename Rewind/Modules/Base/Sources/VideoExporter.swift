...
public final class VideoExporter {
    ...
    public func export(from media: LoadedMedia, options: VideoEditingSettings) async throws -> LoadedMedia {
        guard case let .video(url, _) = media.content else {
            throw NSError(domain: "VideoExportService", code: 1, userInfo: [NSLocalizedDescriptionKey: "Media is not video"])
        }
    ...
    private func exportFromAsset(_ asset: AVURLAsset, options: VideoEditingSettings) async throws -> LoadedMedia {
        ...
        guard let videoTrack = videoTracks.first else {
            throw NSError(domain: "VideoExportService", code: 2, userInfo: [NSLocalizedDescriptionKey: "No video track found"])
        }
        ...
        guard let compositionVideoTrack = composition.addMutableTrack(
            ...
            throw NSError(domain: "VideoExportService", code: 3, userInfo: [NSLocalizedDescriptionKey: "Failed to create video track"])
        }
        ...
        let renderSize = CGSize(width: abs(naturalSize.applying(preferredTransform).width), height: abs(naturalSize.applying(preferredTransform).height))
        ...
        guard let exportSession = AVAssetExportSession(
            ...
            throw NSError(domain: "VideoExportService", code: 4, userInfo: [NSLocalizedDescriptionKey: "Failed to create export session"])
        }
