...
import (
	...
	pb "Rewind-auth-service/pkg/proto" // Путь к вашему сгенерированному proto файлу
)
...
func RunGRPCServer(grpcServer *grpc.Server, authService pb.AuthServiceServer) error {
	listenAddr := os.Getenv("GRPC_PORT")
	if listenAddr == "" {
		listenAddr = ":50051" // Значение по умолчанию
	...
	// Регистрация сервиса AuthService на gRPC сервере
