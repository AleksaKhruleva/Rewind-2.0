...
func RunGRPCServer(grpcServer *grpc.Server, mediaService pb.MediaServiceServer) error {
	listenAddr := os.Getenv("GRPC_PORT")
	if listenAddr == "" {
		listenAddr = ":50055" // Значение по умолчанию
	...
	// Регистрация сервиса MediaService на gRPC сервере
